...
Wasp::Wasp()
{
	...
	mShip = new Texture("Ships/playerShip3_green.png");
	mShip->Parent(this);
	mShip->Pos(VEC2_ZERO);
	...
	mDamages[0] = new Texture("Ships/playerShip3_damage1.png");
	mDamages[1] = new Texture("Ships/playerShip3_damage2.png");
	mDamages[2] = new Texture("Ships/playerShip3_damage3.png");
	...
	mAcceleration = 900.0f;
	mRotationSpeed = 6.0f;

	Reset();

	mEngine[0] = new GameEntity();
	mEngine[1] = new GameEntity();

	mEngine[0]->Parent(this);
	mEngine[1]->Parent(this);

	mEngine[0]->Pos(Vector2(-mShip->ScaledDimensions().x * 0.35f, mShip->ScaledDimensions().y * 0.50f));
	mEngine[1]->Pos(Vector2(mShip->ScaledDimensions().x * 0.35f, mShip->ScaledDimensions().y * 0.50f));
	...
	for (int i = 0; i < MAX_BULLETS; i++)
	{
		...
		mBullets[i]->Speed(1500.0f);
		mBullets[i]->MaxDistance(750.0f);
	}

	mReloadTimer = 0.0f;

	AddCollider(new BoxCollider(Vector2(15.0f, 75.0f)));
	AddCollider(new BoxCollider(Vector2(70.0f, 15.0f), 50.0f), Vector2(23.0f, 4.0f));
	AddCollider(new BoxCollider(Vector2(70.0f, 15.0f), -50.0f), Vector2(-23.0f, 4.0f));
	AddCollider(new BoxCollider(Vector2(70.0f, 28.0f)), Vector2(0.0f, 15.0f));
	AddCollider(new BoxCollider(Vector2(35.0f, 20.0f)), Vector2(0.0f, 30.0f));
...
bool Wasp::IgnoreCollisions()
{
	return !mVisible || mAnimating;
}

void Wasp::HandleMovement()
{
...
void Wasp::Visible(bool visible)
{
	mVisible = visible;
}
...
bool Wasp::DoublePoints()
{
	return mDoublePoints;
}

float Wasp::DoublePointsTimer()
{
	return mDoublePointTimer;
}

bool Wasp::IsAnimating()
{
	return mAnimating;
}
...
void Wasp::Accelerate(bool forward)
{
	...
	{
		mVelocity -= mDirection * mAcceleration * 0.75f * mTimer->DeltaTime();
	}
}

void Wasp::Turn(bool clockwise)
{
	//Add to rotation
	if (clockwise)
	{
